...
class HttpHelper {
    ...
    setCookie(name, value, maxAge, ctx) {
        ctx.cookies.set(name, encodeURIComponent(value), {domain: 'abc.xxx', maxAge: maxAge, httpOnly: false, overwrite: true})
    }
    ...
    deleteCookie(name, ctx) {
        ctx.cookies.set(name, '', {domain: 'abc.xxx', maxAge:0, httpOnly: false, overwrite: true})
    }
